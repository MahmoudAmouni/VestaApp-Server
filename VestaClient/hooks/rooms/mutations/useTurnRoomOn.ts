...
import { apiTurnRoomOn } from "../../../features/rooms/rooms.api";
import { roomsKey } from "../useRoomsQuery";

export function useTurnRoomOn(args: { homeId: number; token?: string }) {
  const { homeId, token } = args;

  return useMutation({
    mutationFn: async (vars: { roomId: number }) => {
      return apiTurnRoomOn({ roomId: vars.roomId, homeId, token });
    },
    onMutate: async ({ roomId }) => {
      ...
      queryClient.setQueryData<Room[]>(roomsKey(homeId), (current) => {
        ...
        return current.map((r) => {
          ...
          return {
            ...r,
            devices: r.devices ? r.devices.map(d => ({ ...d, is_on: true })) : []
          };
        });
      });
